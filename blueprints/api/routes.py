...
from decorators import require_jwt, require_api_key
...
def api_create_profile(uid: str):
    ...
    return jsonify(
        {"message": "Profile saved successfully", "profile": normalized}
    ), 200
...
def api_update_profile(uid: str):
    ...
    if invalid_fields:
        errors.append(
            ...
            "Only first_name, last_name, and student_id are allowed."
        )
    ...
    return jsonify(
        {"message": "Profile updated successfully", "profile": updated_profile}
    ), 200
...
def api_sensor_data(uid: str):
    ...
    return jsonify(
        {"message": "Sensor data received successfully", "id": doc_id}
    ), 201